...
function compactGrokVideo(scene = {}, baseVideo = "") {
  ...
  const camera = cleanPrompt(scene.camera || "handheld documentary fragment");
  const hook = trimWords(visual, 18);
  const action = trimWords(motion, 18);
  const out = `ANIMATE CURRENT FRAME: ${hook}. Single action only: ${action}. Shot like a grounded documentary fragment; ${camera}; tactile atmosphere, real inertia, fabric and particles react naturally. ${EXACT_CONTINUITY}`;
  return trimWords(out, 118);
}
...
function ensureVeoVideo(scene = {}, baseVideo = "") {
  ...
  if (!/Audio:/i.test(out)) {
    out += ` Audio: restrained documentary ambience. SFX: ${scene.sfx || "low room tone, subtle environmental texture"}. No dialogue, no voiceover.`;
  }
  if (!out.includes(EXACT_CONTINUITY)) out += ` ${EXACT_CONTINUITY}`;
  return trimWords(out, 125);
}
