-- NeuroCine V45 — FULL Supabase schema
-- Safe to run multiple times. It DOES NOT delete user data.
-- Covers: profiles, projects, user_settings, usage_events, api_keys, triggers, RLS policies. V45 uses the same DB schema; autosave is frontend/cloud logic.
...
-- Make current owner account full access immediately.
...
set
  ...
  updated_at = now()
where lower(email) in ('dosvidosikml@gmail.com');
