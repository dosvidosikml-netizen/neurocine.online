...
const COVER_PRESETS = [
  ...
  { id: "scifi", label: "Sci-Fi", style: { container: { alignItems: "center" }, hook: { fontSize: 12, fontWeight: 800, fontFamily: "monospace", color: "#34d399", textTransform: "uppercase", letterSpacing: 2, marginBottom: 8 }, title: { fontSize: 32, fontWeight: 900, fontFamily: "monospace", color: "#fff", textTransform: "uppercase", lineHeight: 1.1, textShadow: "0 0 10px #34d399", textAlign: "center" }, cta: { fontSize: 10, border: "1px solid #0ea5e9", color: "#0ea5e9", padding: "4px 8px", marginTop: 10 } } }
];
...
1. PACING: Strictly 3 seconds per scene! Change visual strictly every 3 seconds.
...
4. PROMPTS IN ENGLISH ONLY: Even if the script/target language is Russian, "global_anchor_EN", "imgPrompt_EN", "vidPrompt_EN" and "music_EN" MUST be strictly in ENGLISH. Translate everything (actions, camera, sounds) to English.
5. RESTRICTIONS: KATEGORICHESKI ZAPRESHENO to mention Midjourney or Leonardo. Use ONLY Veo, Whisk, and Grok Super.
...
async function callAPI(content, maxTokens = 8000, sysPrompt = VIRAL_SYSTEM) {
  try {
    ...
    if (!res.ok) throw new Error(`Ошибка API: ${res.status} - ${textRes}`);
...
export default function Page() {
  ...
  function applyResult(rawText, fromHistory = false) {
    ...
    try { 
      ...
      alert("Ошибка генерации. Нейросеть выдала неверный формат (Таймаут). Попробуйте еще раз."); 
      setView("form");
      return;
    }
    ...
    // Формирование итогового скрипта с отступами и чистыми списками промптов
  ...
  async function handleDraftText() {
    if (!topic.trim()) return alert("Введите тему или идею!");
    setBusy(true); 
  ...
  async function handleIntonations() {
    if (!script.trim()) return alert("Нет текста для обработки!");
    setBusy(true); 
  ...
  async function handleTTS() {
    ...
    setBusy(true); 
    setLoadingMsg("Подбор настроек голоса...");
  ...
  async function downloadThumbnail() {
    ...
    function doCapture() {
      window.html2canvas(el, { useCORS: true, scale: 3, backgroundColor: null }).then(canvas => {
        ...
        link.download = `DocuCover_${Date.now()}.png`; 
