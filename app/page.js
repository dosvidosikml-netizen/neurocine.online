...
// После splitLongFrames принудительно доводим до нужного количества кадров.
// AI может вернуть меньше кадров чем запрошено — этот хак гарантирует точное число.
function normalizeToTargetCount(frames = [], targetCount = 20) {
  let result = [...frames];

  // Если кадров МЕНЬШЕ — дробим самые длинные пока не наберём нужное число
  while (result.length < targetCount) {
    let longestIdx = 0, longestDur = 0;
    result.forEach((f, i) => {
      const m = String(f.timecode || "").match(/(\d+)[^0-9]+(\d+)/);
      const dur = m ? parseInt(m[2]) - parseInt(m[1]) : 3;
      if (dur > longestDur) { longestDur = dur; longestIdx = i; }
    });
    if (longestDur <= 2) break;
    const f = result[longestIdx];
    const m = String(f.timecode || "").match(/(\d+)[^0-9]+(\d+)/);
    const s = parseInt(m[1]), e = parseInt(m[2]);
    const mid = s + Math.floor((e - s) / 2);
    result.splice(longestIdx, 1,
      { ...f, timecode: `${s}s–${mid}s` },
      { ...f, timecode: `${mid}s–${e}s` }
    );
  }

  // Если кадров БОЛЬШЕ — удаляем кратчайшие
  while (result.length > targetCount) {
    let shortIdx = result.length - 1, shortDur = Infinity;
    result.forEach((f, i) => {
      const m = String(f.timecode || "").match(/(\d+)[^0-9]+(\d+)/);
      const dur = m ? parseInt(m[2]) - parseInt(m[1]) : 3;
      if (dur < shortDur) { shortDur = dur; shortIdx = i; }
    });
    result.splice(shortIdx, 1);
  }

  // Пересчитываем тайм-коды без пробелов
  let cursor = 0;
  return result.map((frame) => {
    const m = String(frame.timecode || "").match(/(\d+)[^0-9]+(\d+)/);
    const dur = m ? Math.min(4, Math.max(2, Math.abs(parseInt(m[2]) - parseInt(m[1])))) : 3;
    const fixed = { ...frame, timecode: `${cursor}s–${cursor + dur}s` };
    cursor += dur;
    return fixed;
  });
...
BANNED WORDS: "погрузимся", "давайте", "мало кто знает", "история знает", "невероятно", "удивительно", "таким образом", "интересный факт", "можно ли".`);
      ...
      data1A.frames = normalizeToTargetCount(splitLongFrames(allFrames), targetFrames);
  ...
  return (
    <div ref={scrollRef} style={{minHeight:"100vh", color:"#e2e8f0", paddingBottom:100, position:"relative", zIndex:1, overflowY:"auto"}}>
      <NeuralBackground />
      <a href="/storyboard" style={{position:"fixed",right:18,top:18,zIndex:50,textDecoration:"none",padding:"12px 16px",borderRadius:999,background:"linear-gradient(135deg,#7c3aed,#db2777)",color:"white",fontSize:12,fontWeight:900,boxShadow:"0 12px 35px rgba(124,58,237,.35)",letterSpacing:".4px"}}>Storyboard v2</a>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── DESKTOP 2-COL — FIXED RIGHT COLUMN ─────────────────── */
        @media (min-width:1024px) {
          ...
          .studio-right-col .glass { min-width:0; overflow:hidden; }
          .studio-right-col pre,
          .studio-right-col code,
          .studio-right-col textarea,
          .studio-right-col input,
          .studio-right-col button {
            max-width:100%;
          }
