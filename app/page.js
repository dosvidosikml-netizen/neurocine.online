...
СТРУКТУРА JSON ДОЛЖНА БЫТЬ ТАКОЙ:
{
  ...
  "thumbnail": {
    ...
    "prompt": "ENGLISH VEO/WHISK PROMPT: 1 main object, high contrast, 8K"
  }
}
...
async function callAPI(content, maxTokens = 3900, sysPrompt = VIRAL_SYSTEM) {
  try {
    ...
    if (!res.ok) {
      if (res.status === 504) throw new Error("Таймаут (Vercel 10 секунд). Сервер не успел дописать ответ. Нажмите кнопку еще раз!");
      throw new Error(`Ошибка сервера: ${res.status}`);
    }

    const textResponse = await res.text();
    
    // Отлов 504 HTML страницы, если Vercel её вернул со статусом 200 (бывает за прокси)
    if (textResponse.includes("<html") || textResponse.includes("504 Gateway")) {
      throw new Error("Vercel оборвал генерацию по таймауту. Уменьшите длительность видео или нажмите кнопку снова!");
    }
    ...
    try {
      ...
      throw new Error("Сервер прислал неверный формат данных. Попробуйте еще раз.");
    }
  } catch (e) {
    throw e;
  }
...
// УЛУЧШЕННЫЙ ТОСТ - Теперь он огромный и в центре экрана, если это ошибка!
function Toast({ msg, onDone }) {
  ...
  return (
    <div style={{position:"fixed", top:isErr?"35%":"auto", bottom:isErr?"auto":"110px", left:"50%", transform:"translate(-50%, -50%)",
         background:isErr?"rgba(220,38,38,.98)":"rgba(34,197,94,.92)", backdropFilter:"blur(12px)",
         border:`1px solid ${isErr?"#fca5a5":"#86efac"}`, borderRadius:isErr?24:14, padding:isErr?"30px 24px":"10px 18px",
         fontSize:isErr?16:12, fontWeight:800, color:"#fff", zIndex:9999, textAlign:"center",
         boxShadow:isErr?"0 30px 60px rgba(0,0,0,.6)":"none", width:isErr?"90%":"auto", lineHeight:1.5}}>
      {msg}
    </div>
  );
}
...
function LoadingScreen({ msg }) {
  const STEPS = ["Анализирую тему…","Пишу текст диктора...","Строю раскадровку…","Подбираю промпты…","Проверяю тайминг…"];
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"100px 40px",textAlign:"center"}}>
      ...
      <div style={{fontSize:13,color:"rgba(255,255,255,.5)",marginBottom:24}}>{!msg ? STEPS[step] : "Подождите..."}</div>
      {!msg && (
...
export default function Page() {
  ...
  // АЛГОРИТМ-СПАСАТЕЛЬ: Вытащит кадры, даже если Vercel обрежет JSON по таймеру
  function applyResult(rawText, fromHistory = false) {
    ...
    try {
      ...
      console.warn("JSON поврежден (вероятно оборван Vercel), запускаю режим спасения...");
      // Спасаем кадры с помощью регулярных выражений
      ...
      if (data.frames.length === 0) {
        throw new Error("Ответ сервера оборвался слишком рано. Уменьшите длину видео и попробуйте еще раз.");
      }
    }
...
STYLE PROMPT: [English instruction on how to read the text, e.g. "Read with deep suspense and urgency."]`;
  ...
  // ОБНОВЛЕННАЯ ГЕНЕРАЦИЯ: ЗАЩИТА ОТ VERCEL TIMEOUT
  async function handleGenerateFullPlan() {
    ...
    try {
      ...
      if (!currentScript) {
        setLoadingMsg("Сначала пишу сценарий диктору...");
      ...
      setLoadingMsg("Создаю раскадровку и промпты...");
      
      // КОМАНДА ДЛЯ НЕЙРОСЕТИ БЫТЬ МАКСИМАЛЬНО БЫСТРОЙ
      const req = `ВЫДАЙ ОТВЕТ СТРОГО В ФОРМАТЕ JSON. МАССИВ "frames" ДОЛЖЕН СОДЕРЖАТЬ РОВНО ${durCfg.frames} ЭЛЕМЕНТОВ! 
🚨 КРИТИЧЕСКИ ВАЖНО: У НАС ЛИМИТ ВРЕМЕНИ 10 СЕКУНД. 
Пиши МАКСИМАЛЬНО КОРОТКО! Ключи "visual", "imgPrompt", "vidPrompt" — строго 3-7 слов (только суть, без лишних деталей). Иначе видео не сгенерируется!`;
      ...
      const text = await callAPI(fullPrompt, 3900, VIRAL_SYSTEM);
      ...
      {view==="form"&&(
        <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,padding:"16px 18px 32px",background:"linear-gradient(to top,#08080f 60%,transparent)",zIndex:100}}>
          <button className="gbtn" onClick={handleGenerateFullPlan} disabled={(!script.trim() && !topic.trim()) || busy || busyScriptProcess || busyTts}>
            {busy?"Генерирую план…":"▶  Сделать Раскадровку"}
          </button>
        </div>
      )}
    </div>
