...
export default function Page() {
  ...
  const [pageProfile, setPageProfile] = useState({ name: "", genre: "", tone: "", audience: "" });
  ...
  useEffect(() => { 
    if (typeof window !== "undefined") { 
      ...
      // Профиль страницы — загружаем или показываем настройку при первом входе
      const savedProfile = localStorage.getItem("ds_page_profile");
      if (savedProfile) {
        try { setPageProfile(JSON.parse(savedProfile)); } catch(e) {}
      } else {
        // Первый вход — покажем экран настройки после загрузки
        setTimeout(() => setShowPageSetup(true), 800);
      } 
      ...
      const sys = `You are a TTS Director for Google AI Studio. Analyze the script and output ONLY valid JSON (no markdown, no text outside JSON):
{
  ...
  const savePageProfile = (profile) => {
    ...
    localStorage.setItem("ds_page_profile", JSON.stringify(profile));
  ...
  async function handleGenerateFB() {
    ...
    setFbData(null);
    try {
      const pageDNA = pageProfile.name
        ? `СТРАНИЦА: "${pageProfile.name}"\nЖАНР: ${pageProfile.genre}\nТОН: ${pageProfile.tone}\nАУДИТОРИЯ: ${pageProfile.audience}`
        : `СТРАНИЦА: без привязки — пиши универсально`;
...
ДНК СТРАНИЦЫ (обязательно соблюдай стиль):
${pageDNA}

ПРАВИЛА ПУБЛИКАЦИИ FACEBOOK:
— Первая строка = крючок-шок. Читатель должен остановить скролл. Без вступлений.
— Абзацы короткие (2-3 предложения). Много пробелов — легко читать на телефоне.
— Каждый абзац заканчивается на крючке — нельзя остановиться.
— Факты с цифрами, материалами, физикой — никаких абстракций.
— Финал = провокационный вопрос который РАСКАЛЫВАЕТ аудиторию на 2 лагеря → комментарии.
— После текста: 3-5 хештегов тематики страницы. НЕ общие типа #история — конкретные.
— БАН: "невероятно", "удивительно", "мало кто знает", "погрузимся", "итак".

ПРАВИЛА ИСТОРИИ FACEBOOK:
— Максимум 2 предложения. Первое = удар. Второе = крючок "читай публикацию".
— Формат для наложения текста на тёмный фон — короткий, крупный, читается за 2 сек.
— Никаких хештегов в истории.

JSON FORMAT:
{
  "post_hook": "Первая строка публикации — стоп-скроллер",
  "post_body": "Тело публикации. Абзацы разделены через \\n\\n",
  "post_question": "Финальный провокационный вопрос для комментариев",
  "post_tags": "#тег1 #тег2 #тег3 #тег4 #тег5",
  "story_line1": "Первый удар — 1 предложение",
  "story_line2": "Крючок на публикацию — 1 предложение"
}`;
...
BANNED WORDS: "погрузимся", "давайте", "мало кто знает", "история знает", "невероятно", "удивительно", "таким образом", "интересный факт", "можно ли".`);
  ...
  return (
    <div ref={scrollRef} style={{minHeight:"100vh", color:"#e2e8f0", paddingBottom:100, position:"relative", zIndex:1, overflowY:"auto"}}>
      ...
      {view==="result"&&(
        <div style={{maxWidth:640,margin:"0 auto",padding:"20px 16px 120px"}}>
          ...
          <div className="hide-scroll" style={{display:"flex",gap:6,marginBottom:20,overflowX:"auto",paddingBottom:2}}>
            {[["storyboard","🎞 Раскадровка"],["raw","📄 Скрипт и Промпты"],["seo","🚀 Музыка и SEO"],["tts","🎙 TTS Studio"],["fb","📘 Facebook"]].map(([t,label])=>(
          ...
          {tab==="fb"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              {/* Профиль страницы */}
              <div style={{background:"rgba(59,130,246,.06)",border:"1px solid rgba(59,130,246,.25)",borderRadius:16,padding:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  <div style={{fontSize:10,fontWeight:900,color:"#60a5fa",letterSpacing:"2px",textTransform:"uppercase",marginBottom:4}}>📘 СТРАНИЦА</div>
                  <div style={{fontSize:13,color:"#fff",fontWeight:700}}>{pageProfile.name||"Не настроено"}</div>
                  {pageProfile.genre&&<div style={{fontSize:11,color:"#94a3b8",marginTop:2}}>{pageProfile.genre}</div>}
                </div>
                <button onClick={()=>setShowPageSetup(true)} style={{background:"rgba(59,130,246,.15)",border:"1px solid rgba(59,130,246,.4)",borderRadius:10,padding:"8px 14px",color:"#60a5fa",fontSize:11,fontWeight:800,cursor:"pointer"}}>⚙ ИЗМЕНИТЬ</button>
              </div>

              {/* Кнопка генерации */}
              {!fbData&&(
                <div style={{background:"rgba(59,130,246,.07)",border:"1px dashed rgba(59,130,246,.35)",borderRadius:20,padding:20,textAlign:"center"}}>
                  <div style={{fontSize:28,marginBottom:10}}>📘</div>
                  <div style={{fontSize:13,color:"#93c5fd",fontWeight:700,marginBottom:6}}>Контент для Facebook</div>
                  <div style={{fontSize:11,color:"#475569",marginBottom:18,lineHeight:1.6}}>ИИ напишет готовую Публикацию и Историю<br/>в стиле твоей страницы — копируй и публикуй</div>
                  <button onClick={handleGenerateFB} disabled={generatingFB||!script.trim()} style={{width:"100%",padding:"14px",background:"linear-gradient(135deg,#1d4ed8,#1e40af)",borderRadius:14,color:"#fff",fontWeight:900,border:"none",cursor:generatingFB||!script.trim()?"not-allowed":"pointer",opacity:!script.trim()?0.4:1,fontSize:14,letterSpacing:"0.5px",boxShadow:"0 4px 20px rgba(29,78,216,.4)",transition:"all .2s"}}>
                    {generatingFB?"⏳ ГЕНЕРАЦИЯ...":"📘 СГЕНЕРИРОВАТЬ ПУБЛИКАЦИЮ + ИСТОРИЮ"}
                  </button>
                  {!script.trim()&&<div style={{fontSize:10,color:"#ef4444",marginTop:8}}>Сначала создайте сценарий (Шаг 1)</div>}
                </div>
              )}
              ...
              {fbData&&(
                <>
                  {/* ПУБЛИКАЦИЯ */}
                  <div style={{background:"rgba(29,78,216,.06)",border:"1px solid rgba(29,78,216,.3)",borderRadius:16,padding:18}}>
                    <div style={{fontSize:10,fontWeight:900,color:"#60a5fa",letterSpacing:"2px",textTransform:"uppercase",marginBottom:14}}>📢 ПУБЛИКАЦИЯ</div>

                    {/* Превью поста */}
                    <div style={{background:"rgba(0,0,0,.35)",borderRadius:12,padding:16,marginBottom:12,lineHeight:1.7}}>
                      <div style={{fontSize:15,color:"#fff",fontWeight:900,marginBottom:10}}>{fbData.post_hook}</div>
                      <div style={{fontSize:13,color:"#cbd5e1",whiteSpace:"pre-wrap",marginBottom:10}}>{fbData.post_body}</div>
                      <div style={{fontSize:14,color:"#fff",fontWeight:800,marginBottom:10,borderTop:"1px solid rgba(255,255,255,.07)",paddingTop:10}}>{fbData.post_question}</div>
                      <div style={{fontSize:12,color:"#3b82f6"}}>{fbData.post_tags}</div>
                    </div>

                    <CopyBtn
                      text={`${fbData.post_hook}\n\n${fbData.post_body}\n\n${fbData.post_question}\n\n${fbData.post_tags}`}
                      label="📋 СКОПИРОВАТЬ ПУБЛИКАЦИЮ"
                      fullWidth
                    />
                  </div>

                  {/* ИСТОРИЯ */}
                  <div style={{background:"rgba(124,58,237,.06)",border:"1px solid rgba(124,58,237,.3)",borderRadius:16,padding:18}}>
                    <div style={{fontSize:10,fontWeight:900,color:"#a78bfa",letterSpacing:"2px",textTransform:"uppercase",marginBottom:14}}>⚡ ИСТОРИЯ (24 ч)</div>

                    {/* Превью истории */}
                    <div style={{background:"linear-gradient(135deg,rgba(15,5,30,1),rgba(40,10,80,.9))",borderRadius:12,padding:24,marginBottom:12,textAlign:"center",minHeight:120,display:"flex",flexDirection:"column",justifyContent:"center",gap:10,border:"1px solid rgba(168,85,247,.2)"}}>
                      <div style={{fontSize:17,color:"#fff",fontWeight:900,lineHeight:1.4}}>{fbData.story_line1}</div>
                      <div style={{fontSize:14,color:"#d8b4fe",fontWeight:700}}>{fbData.story_line2}</div>
                    </div>

                    <CopyBtn
                      text={`${fbData.story_line1}\n\n${fbData.story_line2}`}
                      label="📋 СКОПИРОВАТЬ ИСТОРИЮ"
                      fullWidth
                    />
                  </div>

                  {/* Сброс */}
                  <button onClick={()=>setFbData(null)} style={{width:"100%",padding:"10px",background:"transparent",border:"1px dashed rgba(255,255,255,.1)",borderRadius:12,color:"#475569",fontSize:11,fontWeight:700,cursor:"pointer"}}>
      ...
      {/* ── PAGE SETUP MODAL ──────────────────────────────────────────── */}
      {showPageSetup&&(
        <div style={{position:"fixed",inset:0,zIndex:10000,background:"rgba(0,0,0,.88)",backdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div className="glass" style={{width:"100%",maxWidth:440,padding:28,position:"relative"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              <h2 style={{fontSize:18,fontWeight:900,color:"#fff"}}>📘 Настрой свою страницу</h2>
              {pageProfile.name&&<button onClick={()=>setShowPageSetup(false)} style={{background:"none",border:"none",color:"#6b7280",fontSize:22,cursor:"pointer"}}>×</button>}
            </div>
            <p style={{fontSize:12,color:"#64748b",marginBottom:20,lineHeight:1.5}}>ИИ будет писать контент именно для твоей аудитории. Один раз — навсегда.</p>

            {[
              {key:"name", label:"📛 Название страницы", placeholder:"Реальные Ужасы Прошлого"},
              {key:"genre", label:"🎭 Тематика / жанр", placeholder:"Тёмная история, забытые трагедии, скрытые факты"},
              {key:"tone", label:"🎙 Тон голоса", placeholder:"Без цензуры, провокационно, о чём молчат учебники"},
              {key:"audience", label:"👥 Целевая аудитория", placeholder:"Люди которые любят жуткое и запретное"},
            ].map(({key,label,placeholder})=>(
              <div key={key} style={{marginBottom:14}}>
                <label style={{fontSize:10,fontWeight:900,color:"#94a3b8",letterSpacing:"1.5px",textTransform:"uppercase",display:"block",marginBottom:6}}>{label}</label>
                <input
                  type="text"
                  value={pageProfile[key]}
                  onChange={e=>setPageProfile(prev=>({...prev,[key]:e.target.value}))}
                  placeholder={placeholder}
                  style={{width:"100%",background:"rgba(0,0,0,.5)",border:"1px solid rgba(168,85,247,.2)",borderRadius:10,padding:"11px 14px",fontSize:13,color:"#fff",fontFamily:"inherit"}}
                />
              </div>
            ))}

            <button
              onClick={()=>savePageProfile(pageProfile)}
              disabled={!pageProfile.name.trim()}
              style={{width:"100%",padding:"14px",background:pageProfile.name.trim()?"linear-gradient(135deg,#1d4ed8,#7c3aed)":"rgba(255,255,255,.05)",borderRadius:14,color:"#fff",fontWeight:900,border:"none",cursor:pageProfile.name.trim()?"pointer":"not-allowed",fontSize:15,marginTop:4,opacity:pageProfile.name.trim()?1:0.4,transition:"all .2s"}}>
