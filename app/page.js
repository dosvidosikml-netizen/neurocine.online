...
2. PIPELINE DIRECTIVE:
    ...
    — ACTION must be derived from the "Visual:" field of the storyboard frame. Use the "Voice:" field as EMOTIONAL CONTEXT ONLY — to understand the mood and physical intensity of the action. Characters never speak on screen.
...
7. AUDIO ANCHOR: END every vidPrompt_EN (and ONLY vidPrompt_EN) with: \`, clear ASMR audio of [sound], isolated sound, zero background noise, no ambient hum.\`
   — CRITICAL: [sound] = COPY VERBATIM the SFX value from the "SFX:" field of the corresponding frame in the storyboard. Example: if storyboard says SFX: "[0:00] Sub Bass Drop + металлический звон" → write "clear ASMR audio of Sub Bass Drop and metallic ring". Do NOT invent or substitute sounds. Translate to English if needed but keep exact meaning.
   — vidPrompt_EN must NEVER contain (token:weight) SD-style negative syntax. That syntax is for imgPrompt_EN only.
