...
// NeuroCine Production Pack v3.3
// Cover Director engine version — bump this when theme detection logic changes to auto-invalidate old cached results.
const COVER_ENGINE_VERSION = "v2.9.1-leper-fix"; — RU/EN Studio UX + premium compact workflow
...
function CoverTab({ topic, script, storyboard, cacheKey, sourceKey, sourceText, devMode, liveAllowed = false, accessToken = "" }) {
  ...
  const staleData = Boolean(
    data && (
      // Engine version mismatch → invalidate cached result (theme detection may have changed)
      ...
      // Legacy: cached "mourning_ritual" for a script that should have been "leper_exile" → force refresh
