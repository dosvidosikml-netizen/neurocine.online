...
export default function PublicLandingV2() {
  ...
  async function loginWithGoogle() {
    ...
    const redirectTo = `${origin}/storyboard`;
  ...
  const authLabel = loading ? "Checking..." : user ? `Studio: ${meta.name}` : busy ? "Opening Google..." : "Войти через Google";
      ...
      <div className="page">
        <header className="fest-header">
          <div className="fest-top">
            ...
            <div></div>
            <div className="right">
              <button className="auth-pill" type="button" onClick={loginWithGoogle} disabled={busy || loading}>
