...
export default function PublicLandingV3() {
  const [session, setSession] = useState(null);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [today, setToday] = useState("2026");
  ...
  return (
    <main className="nc-public-landing nc-public-landing-v3">
      <div className="page">
        <header className="fest-header">
          <div className="fest-top">
            ...
            <div></div>
            <div className="right">
              <button className="auth-pill" type="button" onClick={loginWithGoogle} disabled={busy}>
                <span className="auth-dot" /> {authLabel}
              </button>
              <span>{loading ? "Checking session" : "REC · LIVE"}</span>
          ...
          <div className="fest-sub">
            ...
            <button className="fest-sub-cta" type="button" onClick={openStudio} disabled={busy}>{openLabel} <span>→</span></button>
          </div>
        </header>
        ...
        <section className="final">
          ...
          <div className="final-row">
            ...
            <button className="final-cta" type="button" onClick={openStudio} disabled={busy}>{openLabel} <span>→</span></button>
          </div>
        </section>

        <footer className="colophon"><span>© {new Date().getFullYear()} · <b>NeuroCine</b> · v2.4</span><span className="mid">Set in <b>Archivo Black</b> · Newsreader · Mono</span><span className="right">Next.js · React · <b>OpenRouter</b></span></footer>
      </div>
    </main>
  );
}
