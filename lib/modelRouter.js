...
export async function callOpenRouter({
  ...
  if (!apiKey) {
    return { ok: false, error: "OpenRouter API key not set", model_used: null, payload: null };
  }

  if (signal?.aborted) {
    return { ok: false, error: "Запрос отменён клиентом", model_used: null, aborted: true };
  }

  const baseBody = {
    messages: [
      { role: "system", content: systemPrompt },
      { role: "user", content: userMessage }, // string or array [{type:"text",...},{type:"image_url",...}]
    ],
  ...
  async function tryModel(modelId) {
    try {
      ...
      return { res, payload, aborted: false };
    } catch (err) {
      if (err?.name === "AbortError") {
        return { res: { ok: false, status: 499 }, payload: { error: { message: "Запрос отменён клиентом" } }, aborted: true };
      }
      throw err;
    }
  }
  ...
  // Primary attempt
  let { res, payload, aborted } = await tryModel(config.primary);
  if (aborted) return { ok: false, error: "Запрос отменён клиентом", model_used: config.primary, aborted: true };
  let modelUsed = config.primary;

  // Fallback on failure
  if (!res.ok && config.fallback) {
    if (signal?.aborted) return { ok: false, error: "Запрос отменён клиентом", model_used: modelUsed, aborted: true };
    const fb = await tryModel(config.fallback);
    if (fb.aborted) return { ok: false, error: "Запрос отменён клиентом", model_used: config.fallback, aborted: true };
    if (fb.res.ok) {
      res = fb.res;
      payload = fb.payload;
