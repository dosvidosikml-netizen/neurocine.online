...
export function validateScript(text = "") {
  ...
  const finalTail = sentences.slice(Math.max(0, sentences.length - 3));
  const finalTailText = finalTail.join(" ");
  const conditionalFinal = finalTail.some((s) => CONDITIONAL_FINAL_RE.test(s));
  const nonConditionalFinalText = finalTail.filter((s) => !CONDITIONAL_FINAL_RE.test(s)).join(" ");
  const hasFinalFrameAction = FINAL_FRAME_ACTION_RE.test(nonConditionalFinalText);
  const needsFinalFrame = /\?$/.test(lastSentence) || conditionalFinal;
  if (needsFinalFrame && !hasFinalFrameAction) {
    const source = finalTailText || lastSentence;
    issues.push(`FINAL-FRAME: финал звучит условно или без конкретного кадра-действия героя — "${source.slice(0, 90)}..."`);
