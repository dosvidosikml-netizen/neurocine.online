// NeuroCine access matrix v55 — DEMO / PRO Own Keys / DIRECTOR
// Public users never see ADMIN/BYO/OWNER wording. ADMIN/OWNER is an internal platform role only.
// Security rule: platform API access is email-allowlisted by default.
// Do NOT trust client-editable profile.role/profile.plan for admin unless explicitly enabled server-side.
...
function trustProfileAdminRoles() {
  return String(process.env.NEUROCINE_TRUST_PROFILE_ADMIN_ROLES || "").toLowerCase() === "true";
}
...
export const ROLE_MATRIX = {
  demo: {
    ...
    storageProjects: 3,
    apiSource: "none",
    description: "Бесплатный предпросмотр NeuroCine Studio.",
  },
  free: {
    label: "DEMO",
    plan: "free",
    ...
    storageProjects: 3,
    apiSource: "none",
    description: "Бесплатный режим для знакомства со студией. Полная генерация доступна в PRO.",
  },
  pro: {
    label: "PRO",
    plan: "pro",
    ...
    storageProjects: 100,
    apiSource: "user_keys",
    description: "PRO открывает профессиональную студию и LIVE-генерацию через собственные AI API-ключи пользователя.",
  },
  admin: {
    label: "DIRECTOR",
    plan: "admin",
    publicLabel: "DIRECTOR",
    canLive: true,
    canUseOwnApi: false,
    canUseAdminApi: true,
    needsOwnApiKeys: false,
...
export function normalizeRole(role, plan, email = "") {
  ...
  // SECURITY: profile.role/profile.plan may be readable through user-scoped Supabase.
  // A normal signed-in user must never gain platform API by editing their profile row.
  // By default, admin/owner comes only from the email allowlist above.
  if (trustProfileAdminRoles()) {
    if (rawRole === "owner") return "owner";
    if (rawRole === "admin" || rawPlan === "admin") return "admin";
  }

  // If profile claims admin/owner but email is not allowlisted and trust flag is off,
  // downgrade to free instead of pro. This prevents platform API escalation.
  if (rawRole === "owner" || rawRole === "admin" || rawPlan === "admin") return "free";
